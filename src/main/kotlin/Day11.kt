...
object Day11 {
    ...
    fun A(): Int {
        ...
        while (!m.hasHalted) {
            ...
            m.inputStream.add(colors[loc] ?: 0)
            m.run()

            for (p in m.outputStream.windowed(2, 2)) {
                val paint = p[0]
                val direction = p[1]
                colors[loc] = paint
                dir = if (direction == 0L) {
    ...
    fun B(): String {
        ...
        while (!m.hasHalted) {
            ...
            m.inputStream.add(colors[loc] ?: 1)
            m.run()
        ...
        val bounds = colors.keys.fold(listOf(0, 0, 0, 0)) { a, b ->
            listOf(
                min(a[0], b.x), min(a[1], b.y),
                max(a[2], b.x), max(a[3], b.y)
            )
        }

        val out = StringBuilder()

        (bounds[3] downTo bounds[1]).forEach { y ->
            (bounds[0]..bounds[2]).forEach { x ->
                out.append(if ((colors[Point(x, y)] ?: 0) == 1L) "[]" else "  ")
            }
            out.append('\n')
        }
